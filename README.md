...
v0.6.0 是一个本地优先、默认 dry-run、支持每日 8 点自动草稿箱写入的公众号 AI 内容生产流水线。它已经串起从 AI 资讯采集、选题、事实包、文章、标题优化、审核、真实 APIMart 封面、公众号 HTML 排版、最终预检，到微信公众号官方 API 草稿箱写入的完整链路，并保留真实草稿 same-day 运行锁、失败通知和每日运行报告。
...
14. `pnpm run:daily:auto` 可串联每日生产、真实数据审计、mock 草稿、官方 API dry-run、最终预检和真实草稿箱写入，并输出 `outputs/daily-auto-report.md`、`outputs/daily-auto-result.json`、`logs/daily-auto.log` 与 `runs/yyyy-mm-dd-HHmmss/run-report.md`。
15. `pnpm scheduler:install` / `pnpm scheduler:show` / `pnpm scheduler:uninstall` 管理项目专属 cron 块，默认每天早上 8 点运行。
...
- 不默认操作微信公众号后台页面。
- 不内置常驻定时服务；定时运行只通过系统 cron 调用 `pnpm run:daily:auto`。
- 不接数据库。
...
| `pnpm scheduler:install` | 安装项目专属 cron，默认每天早上 8 点运行 |
