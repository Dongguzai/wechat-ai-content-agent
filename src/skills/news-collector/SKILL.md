...
- 采集后的资讯标题、搜索 query、摘要和正文片段必须使用中文；OpenAI、Codex、Claude Code 等固定专名和 API、SDK、LLM、RAG、R2 等常见缩写允许保留英文，其他普通说明词必须中文化，否则 hard rejection。
...
实现必须放在 `src/adapters/` 下，再由 `src/pipeline/collectNews.ts` 调用。采集结果必须经过 `requireSourceUrl` 和 `requireChineseNewsLanguage` 校验。
